...
/**
 * @return true if player has an interact range actor, but is not holding an actor
 */
...
/**
 * Gets backwalk animation for parallel backwalk
 */
    ...
    if (!Player_CheckMeleeInterrupt(play, this)) {
        ...
        if (LinkAnimation_Update(play, &this->skelAnime)) {
            ...
            if (!Player_ActionHandler_MeleeBottleFish(this, play)) {
                ...
                Player_SetupIdlePlayGivenAnim(this, postAttackAnim, play); // Play animation + set new action function
