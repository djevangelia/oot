...
s32 EnSw_HandleDamage(EnSw* this, PlayState* play) {
    ...
    if (this->ACColliderTimer == 0) {
        if ((this->collider.base.acFlags & AC_HIT) || hammerWave) {
            ...
            Actor_SetColorFilter(&this->actor, COLORFILTER_COLORFLAG_RED, 200, COLORFILTER_BUFFLAG_OPA, this->ACColliderTimer);
...
void EnSw_GS_RotateSound(EnSw* this, PlayState* play) {
    if (!(this->actor.scale.x < 0.0139999995f)) {
        ...
        if (!(Math_Vec3f_DistXYZ(&this->actor.world.pos, &activeCam->eye) >= 380.0f)) {
            Actor_PlaySfx(&this->actor, (ENSW_TYPE(this) > ENSW_SKULLWALLTULA) ? NA_SE_EN_STALGOLD_ROLL
...
s32 EnSw_S_ShouldTarget(EnSw* this, PlayState* play, s32 trueArg) {
    ...
    if (!(player->stateFlags1 & PLAYER_STATE1_CLIMBING) && trueArg) {
        return false;
    ...
    } else if (!BgCheck_EntityLineTest1(&play->colCtx, &this->actor.world.pos, &player->actor.world.pos, &posResult, &outPoly,
...
s32 EnSw_S_SightCheck(EnSw* this, PlayState* play) {
    ...
    if (this->collider.base.ocFlags1 & OC1_HIT) {
        ...
        result = false;
               ...
               !BgCheck_EntityLineTest1(&play->colCtx, &this->actor.world.pos, &this->sightLine1S, &posResult, &outPoly, true,
                                        false, false, true, &bgId)) {
        result = false;
    } else if (((play->state.frames % 4) == 1) &&
               BgCheck_EntityLineTest1(&play->colCtx, &this->actor.world.pos, &this->sightLine2S, &posResult, &outPoly, true, false,
                                       false, true, &bgId)) {
        result = false;
    } else if (((play->state.frames % 4) == 2) &&
               !BgCheck_EntityLineTest1(&play->colCtx, &this->actor.world.pos, &this->sightLine3S, &posResult, &outPoly, true,
                                        false, false, true, &bgId)) {
        if (0) {}
        result = false;
    } else if (((play->state.frames % 4) == 3) &&
               BgCheck_EntityLineTest1(&play->colCtx, &this->actor.world.pos, &this->sightLine4S, &posResult, &outPoly, true, false,
                                       false, true, &bgId)) {
        result = false;
    }

    // Adjust position against wall
    if (BgCheck_EntityLineTest1(&play->colCtx, &this->actor.world.pos, &this->bodyLineS, &posResult, &this->wallPolyS, true,
                                false, false, true, &bgId)) {
...
void EnSw_ActionS_TargetAttack(EnSw* this, PlayState* play) {
    ...
    // Attacking
        ...
        // Move quickly to attack
            ...
            if (!(Math_Vec3f_DistXYZ(&this->actor.world.pos, &this->targetPosS) > 13.0f) || Player_IsClimbingStill(play)) {
