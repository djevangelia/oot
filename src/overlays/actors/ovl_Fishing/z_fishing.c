...
void Fishing_DrawRod(PlayState* play2) {
    ...
    if ((sRodCastState == 3) || (sRodCastState == 4)) {
        ...
        lureXZLen = player->wv2.fishingVar2;
        Math_SmoothStepToF(&player->wv2.fishingVar2, input->rel.stick_y * 0.02f, 0.3f, 5.0f, 0.0f);
        lureXZLen = player->wv2.fishingVar2 - lureXZLen;

        spC4 = player->wv1.fishingVar1;
        Math_SmoothStepToF(&player->wv1.fishingVar1, input->rel.stick_x * 0.02f, 0.3f, 5.0f, 0.0f);
        spC4 = player->wv1.fishingVar1 - spC4;

        if (player->wv1.fishingVar1 > 1.0f) {
            player->wv1.fishingVar1 = 1.0f;
        }
        if (player->wv2.fishingVar2 > 1.0f) {
            player->wv2.fishingVar2 = 1.0f;
        }
        if (player->wv1.fishingVar1 < -1.0f) {
            player->wv1.fishingVar1 = -1.0f;
        }
        ...
        Math_ApproachZeroF(&player->wv2.fishingVar2, 1.0f, 0.1f);
        Math_ApproachZeroF(&player->wv1.fishingVar1, 1.0f, 0.1f);
    ...
    Matrix_RotateZ((player->wv1.fishingVar1 * 0.5f) + 3.0f * M_PI / 20.0f, MTXMODE_APPLY);
    Matrix_RotateX((D_80B7A6C0 + 20.0f) * 0.01f * M_PI, MTXMODE_APPLY);
    Matrix_Scale(0.70000005f, 0.70000005f, 0.70000005f, MTXMODE_APPLY);

    spC0 = D_80B7A6AC + D_80B7A6B8 + (D_80B7A6BC * ((((*player).wv2.fishingVar2 - 1.0f) * -0.25f) + 0.5f));
    ...
    switch (sRodCastState) {
        case 0:
            ...
            if (player->stateFlags1 & PLAYER_STATE1_IN_WATER) {
                sRodCastTimer = 0;
                player->wv3.fishingRodState = 0;
            }

            if (sRodCastTimer == 0) {
                if ((D_80B7E0B0 == 0) && (player->wv3.fishingRodState == 1)) {
                    sRodCastTimer = 37;
                    Message_CloseTextbox(play);
                }
            } else {
