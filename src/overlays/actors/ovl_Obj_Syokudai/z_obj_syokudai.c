...
void ObjSyokudai_Update(Actor* thisx, PlayState* play2) {
    ...
    if (WaterBox_GetSurfaceImpl(play, &play->colCtx, this->actor.world.pos.x, this->actor.world.pos.z, &waterSurface,
        ...
        if (interactionType != 0) {
            if (this->litTimer != 0) {
                if (interactionType < 0) {
                    if (player->wv3.dekuStickTimer == 0) {
                        player->wv3.dekuStickTimer = 210;
                        SFX_PLAY_AT_POS(&this->actor.projectedPos, NA_SE_EV_FLAME_IGNITION);
                    } else if (player->wv3.dekuStickTimer < 200) {
                        player->wv3.dekuStickTimer = 200;
                    }
                ...
                if ((0 <= this->litTimer) && (this->litTimer < (50 * litTimeScale + 100)) && (torchType != 0)) {
                    this->litTimer = 50 * litTimeScale + 100;
                                            ...
                                            ((interactionType < 0) && (player->wv3.dekuStickTimer != 0)))) {

                if ((interactionType < 0) && (player->wv3.dekuStickTimer < 200)) {
                    player->wv3.dekuStickTimer = 200;
                }
                if (torchCount == 0) {
                    this->litTimer = -1;
