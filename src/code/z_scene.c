...
/**
 * Run every frame. Loads new unloaded objects on scene init/room change, whose data have been added to the
 * object slot array by `func_800982FC`.
 */
void Object_UpdateEntries(ObjectContext* objectCtx) {
    ...
    for (i = 0; i < objectCtx->numEntries; i++) {
        if (entry->id < 0) { // Not loaded object
...
/**
 * Get object slot for a given object id, if the object is loaded.
 * ABS(slot.id) is necessary if the object is not yet loaded.
 ...
 * Sync DMA loads all loaded objects.
 */
 ...
 * objects.
 * @note There is an assert to ensure that the next object slot starts within the object memory space,
 * but in non-debug this is not included and there is no bounds check.
 * @return void* to the start of the next object slot
 */
...
BAD_RETURN(s32) Scene_CommandObjectList(PlayState* play, SceneCmd* cmd) {
    ...
    // on the same slot doesn't have the same id. Also kill dependent actors.
    ...
    // Add objects from new object list, with their data starting in object memory space `nextptr`
