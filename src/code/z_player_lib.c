...
void func_8008F87C(PlayState* play, Player* this, SkelAnime* skelAnime, Vec3f* pos, Vec3s* rot, s32 thighLimbIndex,
    ...
    if ((this->actor.scale.y >= 0.0f) && !(this->stateFlags1 & PLAYER_STATE1_DEAD) &&
        ...
        sp74 = D_80126068[(void)0, gSaveContext.save.linkAge] - this->quicksandEffect;
    ...
    if (limbIndex == PLAYER_LIMB_L_HAND) {
        ...
        if (this->itemAction == PLAYER_IA_DEKU_STICK) {
            ...
            if (this->actor.scale.y >= 0.0f) {
                sMeleeWeaponTipOffsetFromLeftHand0.x = this->wv2.dekuStickLength * 5000.0f;
            ...
            Matrix_Scale(1.0f, this->wv2.dekuStickLength, 1.0f, MTXMODE_APPLY);
        ...
        if (this->rightHandType == PLAYER_MODELTYPE_RH_FF) {
            ...
            // - Positive wv3.rangedWeaponState means the weapon has actually been loaded since entering aiming mode (it is set
            ...
            if ((this->stateFlags1 & PLAYER_STATE1_RANGED_WEAPON_LOADED) && (this->wv3.rangedWeaponState >= 0) &&
                ...
                this->wv1.stringReboundVar1 = distXYZ - 3.0f;
                if (distXYZ < 3.0f) {
                    this->wv1.stringReboundVar1 = 0.0f; // Neutral
                } else {
                    this->wv1.stringReboundVar1 *= 1.6f;
                    if (this->wv1.stringReboundVar1 > 1.0f) {
                        this->wv1.stringReboundVar1 = 1.0f; // Fully drawn
                    }
                }

                this->wv2.stringReboundVar2 = -0.5f;
            }

            // In Player_UpdateCommon, if player is holding a ranged weapon, Player_StringReboundCalculation is run
            // (before this function). This sets wv1.stringReboundVar1 depending on previous value (and wv2.stringReboundVar2). If the weapon is not
            // loaded and wv1.stringReboundVar1 set above, this value for wv1.stringReboundVar1 is used for drawing and creates a string rebound
            // effect.
            ...
            Matrix_Scale(1.0f, this->wv1.stringReboundVar1, 1.0f, MTXMODE_APPLY);

            // For Slingshot, pull the string downwards (closer to Link, along the Slingshot)
            if (!LINK_IS_ADULT) {
                Matrix_RotateZ(this->wv1.stringReboundVar1 * -0.2f, MTXMODE_APPLY);
            }
